...
class BlockTreeProcessor:
    ...
    def _build_templates(
        ...
        tree_mask = torch.zeros((self.block_size, self.block_size), dtype=torch.bool)
        for node_idx in range(self.block_size):
            cur = node_idx
            while cur >= 0:
        ...
        for q_idx in range(self.block_size):
            for k_idx in range(self.block_size):
                if q_idx == k_idx:
    ...
    def _compute_cum_probs(self, probs_slice: torch.Tensor) -> torch.Tensor:
        cum_probs = torch.zeros((self.block_size,), dtype=torch.float32)
        primary_cumprob = 1.0
        for depth_idx in range(self.tree_seq_depth):
            root_prob = float(probs_slice[depth_idx, 0].item())
            primary_cumprob *= root_prob
            base = depth_idx * self.subtree_size
            cum_probs[base] = primary_cumprob
            for vertex_idx in range(1, self.subtree_size):
                path_prob = primary_cumprob
                cur = vertex_idx
                local_path: list[int] = []
                while cur != 0:
                ...
                for node_idx in reversed(local_path):
                    path_prob *= float(probs_slice[depth_idx, node_idx].item())
                cum_probs[base + vertex_idx] = path_prob
        return cum_probs
    ...
    def build_anchor_tensors(
        ...
        *,
        ...
        for anchor_idx, (anchor_local, anchor_position) in enumerate(zip(anchor_local_positions, anchor_positions)):
            tree_slice = response_subtrees[anchor_local : anchor_local + self.tree_seq_depth]
            prob_slice = response_probs[anchor_local : anchor_local + self.tree_seq_depth]
            cum_probs = self._compute_cum_probs(prob_slice)

            for depth_idx in range(self.tree_seq_depth):
                for vertex_idx in range(self.subtree_size):
                    flat_idx = depth_idx * self.subtree_size + vertex_idx
                    token = int(tree_slice[depth_idx, vertex_idx].item())
